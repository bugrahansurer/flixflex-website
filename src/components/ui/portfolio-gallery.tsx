...
export function PortfolioGallery({
  ...
  return (
    <section
      ...
      id="archives"
    >
      <div className="bg-background/50 backdrop-blur-sm border-b border-border overflow-hidden">
        {/* Header Section */}
        <div className="relative z-10 text-center pt-16 pb-8 px-8">
          <h2 className="text-2xl md:text-3xl font-display font-black text-(var(--foreground)) mb-8">{title}</h2>

          <Link
            href={archiveButton.href}
            className="ff-shape-button ff-btn-primary inline-flex items-center gap-3 px-6 py-3 font-medium mb-20"
          >
        ...
        <div className="ff-shape-button hidden md:block relative overflow-hidden h-[400px] -mb-[200px]">
          <div className={`flex ${spacing} pb-8 pt-40 items-end justify-center`}>
            {images.map((image, index) => {
              ...
              return (
                <motion.div
                  key={index}
                  className="ff-shape-button shadow-2 xl group cursor-pointer flex-shrink-0"
                  style={{
                    zIndex: zIndex,
                  }}
                  initial={{
...
export function DemoPortfolioGallery({ items }: { items?: any[] }) {
  const customImages = items && items.length > 0 ? items.map(item => ({
    src: item.coverImage || "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&h=600&fit=crop&q=80",
    alt: item.title,
    title: item.title
  })) : undefined;

  return <PortfolioGallery images={customImages} />;
}
